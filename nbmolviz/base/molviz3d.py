...
class MolViz3D(MessageWidget):
    ...
    def _grid_to_cube(grid,f=None):
        ...
        print('-1 %f %f %f' % grid.origin(), file=fobj)
        # lines 4-7: number of points in each direction and basis vector for each
        # basis vectors are negative to indicate angstroms
        print('%d %f 0.0 0.0' % (-grid.npoints, grid.dx), file=fobj)
        print('%d 0.0 %f 0.0' % (-grid.npoints, grid.dy), file=fobj)
        print('%d 0.0 0.0 %f' % (-grid.npoints, grid.dz), file=fobj)
