...
class MolViz3D(MessageWidget):
    ...
    def _grid_to_cube(grid,f=None):
        ...
        print('-1 %f %f %f' % grid.origin().value_in(u.angstrom), file=fobj)
        # lines 4-7: number of points in each direction and basis vector for each
        # basis vectors are negative to indicate angstroms
        print('%d %f 0.0 0.0' % (-grid.npoints, grid.dx.value_in(u.angstrom)), file=fobj)
        print('%d 0.0 %f 0.0' % (-grid.npoints, grid.dy.value_in(u.angstrom)), file=fobj)
        print('%d 0.0 0.0 %f' % (-grid.npoints, grid.dz.value_in(u.angstrom)), file=fobj)
        ...
        for ix in range(grid.npoints):
            for iy in range(grid.npoints):
                for iz in range(grid.npoints):
                    print(grid.fxyz[ix, iy, iz], end=' ', file=fobj)
                    # ival += 1
                    # if ival%6 == 0: print >> fobj #newline
                    if iz % 6 == 5: print(file=fobj)
                print(file=fobj)
